...
3- ff bilgisinde üzgün surat ikonu çıkart , diğerlerine ise gülücük olsun
...
if (examGrade >= 0 && examGrade <= 100) {
    ...
    if (examGrade >= 90){
        ...
        textInfo = `${EXPRESSIONLESS} DC`
        info.classList.remove("text-success")
        info.classList.add("text-primary")
    } else if (examGrade >= 60){
        textInfo = `${EXPRESSIONLESS} DD`
        info.classList.remove("text-success")
        info.classList.add("text-primary")
    } else if (examGrade >= 50){
        textInfo = `${EXPRESSIONLESS} FD`
        info.classList.remove("text-success")
        info.classList.add("text-primary")
    } else if (examGrade < 50){
        textInfo = `${FROWN} FF`
        info.classList.remove("text-success","text-primary")
        info.classList.add("text-danger")
    }
} else {
    textInfo = "Girdiğiniz bilgiler doğru değil"
}
